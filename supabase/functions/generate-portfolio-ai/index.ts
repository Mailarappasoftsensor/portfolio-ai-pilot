...
serve(async (req) => {
  ...
  try {
    ...
    const prompt = buildPrompt(generationType, inputData);
    ...
    const groqResponse = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert portfolio and resume writer. Generate professional, engaging content that highlights achievements and skills. Always respond with valid JSON format.'
          },
          {
            role: 'user',
            content: prompt
...
function buildPrompt(generationType: string, inputData: any): string {
  const { jobTitle, industry, experience, skills, resumeText, tone = 'professional' } = inputData;

  if (generationType === 'full_portfolio') {
    return `Generate a complete portfolio content in JSON format for a ${jobTitle} in ${industry} with ${experience} years of experience.
...
Return JSON with this exact structure:
{
  "hero": {
    ...
    "description": "Compelling 2-3 sentence elevator pitch"
  },
  "about": {
    "content": "Detailed 2-3 paragraph professional summary highlighting expertise, achievements, and career goals"
  },
  "experience": [
    {
      "id": "1",
...
Make the content specific, professional, and achievement-focused. Include realistic but impressive details.`;
  }

  return `Generate ${generationType} content for a ${jobTitle} portfolio. Return valid JSON format.`;
}
